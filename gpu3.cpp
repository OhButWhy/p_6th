...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("check", po::value<int>(&check_period)->default_value(check_period), "Check convergence every K iterations")
    ...
    int left_top = 10, left_bottom = 20, right_top = 20, right_bottom = 30;
    ...
    local_grid[ny * nx - 1] = 30; local_newgrid[ny * nx - 1] = 30;
    ...
    if (!write_matrix_text(output_path, final_grid, N)) {
        ...
        delete[] local_grid; delete[] local_newgrid; return 2;
    }
    std::cout << "matrix_file: " << output_path << std::endl;

    delete[] local_grid; delete[] local_newgrid;
    return 0;
}
